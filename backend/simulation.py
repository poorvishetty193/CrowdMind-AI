...
class Simulator:
    def __init__(self):
        ...
        self.ai_telemetry = {
            "total_predictions": 0,
            "total_decisions": 0,
            "wait_time_reduced_percentage": 0,
            "crowd_balanced_percentage": 0
        }
        
    def calculate_wait_times(self) -> int:
        """Computes wait times based on occupancy using a non-linear exponential formula."""
        total_wait = 0
        for zone in self.state.zones:
            occupancy_ratio = zone.current_occupancy / zone.capacity if zone.capacity > 0 else 0
            # Non-linear scaling: wait time spikes dramatically as zone reaches capacity
            zone.wait_time_mins = int(5 + (occupancy_ratio ** 3) * 40)
            total_wait += zone.wait_time_mins
        return total_wait
